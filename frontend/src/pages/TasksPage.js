...
import { Plus, GripVertical, Calendar, Trash2, Edit2 } from 'lucide-react';
          ...
          return (
            <div key={col.key} className={`kanban-column border-t-2 ${col.color}`} data-testid={`kanban-col-${col.key}`}>
              ...
              <div className="space-y-2 min-h-[100px]">
                {colTasks.map(task => (
                  <Card key={task.id} className="bg-background/60 backdrop-blur border-border/40 hover:border-primary/30 transition-all cursor-pointer group" data-testid={`task-card-${task.id}`}>
